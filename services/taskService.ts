...
export async function getTasks(userId: string): Promise<Task[]> {
    ...
    const taskRows = tasks as unknown as Tables<'tasks'>[];
    ...
    const seoReportPageIds = [...new Set(((seoReports as unknown as Tables<'seo_reports'>[]) || []).map(r => r.page_id).filter(Boolean))] as string[];
    ...
    const teamMembersMap = new Map(((teamMembers as unknown as Tables<'team_members'>[]) || []).map(i => [i.id, i.name]));
    const pagesMap = new Map(((pages as unknown as Tables<'pages'>[]) || []).map(i => [i.id, i.name]));
    const ordersMap = new Map(((orders as unknown as Tables<'orders'>[]) || []).map(i => [i.id, `#${i.id}`]));
    const bookingsMap = new Map(((bookings as unknown as Tables<'bookings'>[]) || []).map(i => [i.id, `Booking #${i.id.substring(0, 8)}`]));
    const proofingRequestsMap = new Map(((proofingRequests as unknown as Tables<'proofing_requests'>[]) || []).map(i => [i.id, i.title]));
    const contactsMap = new Map(((contacts as unknown as Tables<'contacts'>[]) || []).map(i => [i.id, i.name]));
    const productsMap = new Map(((products as unknown as Tables<'products'>[]) || []).map(i => [i.id, i.name]));
    const seoPageMap = new Map(((seoPages as unknown as Tables<'pages'>[]) || []).map(p => [p.id, p.name]));
    const seoReportsMap = new Map(((seoReports as unknown as Tables<'seo_reports'>[]) || []).map(i => [i.id, `SEO Report for ${seoPageMap.get(i.page_id) || 'page'}`]));
    const pageGroupsMap = new Map(((pageGroups as unknown as Tables<'page_groups'>[]) || []).map(i => [i.id, i.name]));
    
    // 5. Map over tasks to enrich them
    return taskRows.map(task => ({
        ...task,
        subtasks: task.subtasks as unknown as TaskSubtask[] | null,
...
export async function createTask(userId: string, taskData: Omit<Task, 'id' | 'user_id' | 'created_at' | 'updated_at'>): Promise<Task> {
    ...
    return { ...task, subtasks: task.subtasks as unknown as TaskSubtask[] | null } as Task;
}
