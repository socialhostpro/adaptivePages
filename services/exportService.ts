...
import type { LandingPageData, ImageStore, ManagedProduct, ProductOption, CrmForm, HeroSlide, HeroButton, ManagedPage } from '../types';
...
const generateHeadHTML = (data: LandingPageData, imagePathMapping: Record<string, string>): string => {
    ...
    if (seo.schemaSnippet) {
        // The snippet already contains the script tag usually
    ...
    const backgroundMediaHTML = () => {
        if (hero.backgroundType === 'video') {
            const videoSrc = (hero.videoSource === 'vimeo' && hero.vimeoVideoId)
                ? `https://player.vimeo.com/video/${hero.vimeoVideoId}?background=1&autoplay=1&loop=1&byline=0&title=0`
