...
export async function getProducts(userId: string): Promise<ManagedProduct[]> {
    ...
    const categoryMap = new Map(((categories || []) as unknown as CategoryRow[]).map(c => [c.id, c.name]));
...
export async function updateProduct(productId: string, updates: Partial<Omit<ManagedProduct, 'id' | 'user_id' | 'created_at'>>): Promise<ManagedProduct> {
    ...
    const { data: categoryData, error: catError } = await supabase.from('product_categories').select('id, name').eq('id', (data as unknown as ProductRow).category_id!).single();
    if(catError) console.warn("Could not fetch category name for updated product");

    const categoryMap = new Map(categoryData ? [[(categoryData as unknown as CategoryRow).id, (categoryData as unknown as CategoryRow).name]] : []);

    return mapProduct(data as unknown as ProductRow, categoryMap);
}
