...
import type { CourseSectionData, LandingPageTheme, ImageStore, CourseChapter, CourseLesson } from '../types';
...
export const LessonViewerModal: React.FC<LessonViewerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[102] flex items-center justify-center p-0 md:p-4" onClick={onClose}>
      <div
        className="bg-white dark:bg-slate-900 w-full h-full md:rounded-2xl md:shadow-2xl md:max-w-7xl md:h-[90vh] flex flex-col md:flex-row-reverse overflow-hidden md:border dark:border-slate-700"
        onClick={e => e.stopPropagation()}
      >
        {/* Right Sidebar */}
        <div className="md:w-1/3 p-6 flex flex-col space-y-4 border-t md:border-t-0 md:border-l dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 overflow-y-auto order-last md:order-none">
                        ...
                        {isExpanded && (
                            <ul className="pl-4 mt-1 space-y-1">
                                {chapter.lessons.map(l => {
                                    ...
                                    const isLocked = isSequentiallyLocked;
        ...
        {/* Main Content */}
        <div className="md:w-2/3 flex-shrink-0 bg-black flex flex-col min-h-0 order-first md:order-none">
          ...
          <div className="flex-grow flex flex-col min-h-0">
              ...
              <div className="flex items-center gap-4 pt-4">
                {lesson.worksheetUrl && (
                    <a 
                        ...
                        download
                        className={`flex items-center justify-center gap-2 py-3 px-4 rounded-lg font-semibold text-lg text-${theme.primaryColorName}-700 dark:text-${theme.primaryColorName}-300 bg-${theme.primaryColorName}-100 dark:bg-${theme.primaryColorName}-500/20 hover:bg-${theme.primaryColorName}-200 dark:hover:bg-${theme.primaryColorName}-500/30 transition`}>
                        <DownloadIcon className="w-5 h-5" />
                        Worksheet
                    </a>
                )}
                
                {allLessonsInChapterCompleted && currentChapter.quiz ? (
                    <button 
                        onClick={() => { /* This should be handled by onTakeQuiz from the parent */ }}
                        className={`flex-grow py-3 px-4 rounded-lg font-semibold text-lg text-white bg-indigo-600 hover:bg-indigo-700 transition`}
                    >
                        Take Chapter Quiz
                    </button>
                ) : !isCompleted ? (
                    <button 
                        ...
                        className={`flex-grow py-3 px-4 rounded-lg font-semibold text-lg text-white bg-${theme.primaryColorName}-600 hover:bg-${theme.primaryColorName}-700 transition`}>
                        Mark as Complete
                    </button>
                ) : (
                    <p className="w-full text-center font-semibold text-green-600 dark:text-green-400 p-3 rounded-lg bg-green-50 dark:bg-green-500/10">Lesson Completed!</p>
                )}
              </div>
               {allLessonsCompleted && course.certificateEnabled && (
                <div className="pt-4 border-t dark:border-slate-600 text-center">
                    <h4 className="font-bold text-lg text-yellow-500">Course Complete!</h4>
                     <input 
                        type="text"
                        placeholder="Enter your name for certificate"
                        value={studentName}
                        onChange={(e) => setStudentName(e.target.value)}
                        className="mt-2 w-full max-w-sm mx-auto p-2 border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200"
                    />
                    <button 
                        onClick={() => setShowCertificate(true)}
                        disabled={!studentName}
                        className={`mt-2 py-2 px-4 rounded-lg font-semibold text-sm flex items-center justify-center gap-2 text-white bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-400 w-full max-w-sm mx-auto`}
                    >
            ...
            <footer className="p-3 border-t dark:border-slate-700 flex justify-between items-center flex-shrink-0 bg-white dark:bg-slate-900">
                <button
                    onClick={() => handleNavigate('prev')}
                    disabled={currentLessonIndex === 0}
                    className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 disabled:opacity-50"
                >
                    <ChevronLeftIcon className="w-5 h-5" /> <span className="text-sm font-semibold hidden md:inline">Previous</span>
                </button>
                <div className="text-sm font-semibold text-gray-800 dark:text-slate-200 truncate px-4">
                    {lesson.title}
                </div>
                <button
                    ...
                    className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 disabled:opacity-50"
                >
                    <span className="text-sm font-semibold hidden md:inline">Next</span> <ChevronRightIcon className="w-5 h-5" />
                </button>
            </footer>
        </div>
      </div>
