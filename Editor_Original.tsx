...
import { generateLandingPageStructure, generateAndUploadImage, generateNewSection } from './services/geminiService';
...
export default function Editor({ session }: EditorProps): React.ReactElement {
    ...
    useEffect(() => {
        const initializeEditor = async () => {
            ...
            // Check if there's a specific page to load from localStorage (from wizard)
            const pageIdToLoad = localStorage.getItem('loadPageId');
            if (pageIdToLoad) {
                console.log('🎯 Found pageId in localStorage, loading:', pageIdToLoad);
                localStorage.removeItem('loadPageId'); // Clear it after use
                if (pages && pages.length > 0) {
                    await loadPageById(pageIdToLoad, pages);
                } else {
                    console.warn('📋 No pages loaded, cannot find page:', pageIdToLoad);
                }
            }
        };
    ...
    useEffect(() => {
        // Always show dashboard on first load - let user choose what to do
        if (managedPages.length > 0 && !activePage) {
            console.log('📊 Pages loaded, keeping dashboard open for user to choose');
            setDashboardOpen(true);
        } else if (managedPages.length === 0 && !isPagesLoading && !activePage) {
            console.log('📊 No pages found, opening dashboard for user to create');
            setDashboardOpen(true);
        }
                     ...
                     if (imageKeys.length > 0) {
                        const imagePrompt = (sectionData as any).imagePrompt || (sectionData as any).items?.[0]?.avatarImagePrompt || (sectionData as any).items?.[0]?.imagePrompt;
                         if(imagePrompt && !imagePrompt.startsWith('http')) {
                            const imageUrl = await generateAndUploadImage(imagePrompt, imageKeys[0], session.user.id);
                            newImages[imageKeys[0]] = imageUrl;
                        }
                    }
                }
    ...
    const handleGenerate = useCallback(async (skipPageCheck = false) => {
        ...
        try {
            ...
            console.log('📊 LandingPageData before setting:', landingPageData);
            setLandingPageData(finalData);
            console.log('🎯 LandingPageData set successfully in Editor');
            if (finalData.sectionOrder) {
                console.log('📋 Setting section order:', finalData.sectionOrder);
                setSectionOrder(finalData.sectionOrder);
            }
            
            // Image generation is now separate and won't affect the main page data
            try {
                ...
                if (promptsToGenerate.length > 0) {
                    ...
                    console.log('📸 Starting image generation for prompts:', promptsToGenerate.map(p => ({ key: p.key, prompt: p.prompt.substring(0, 50) + '...' })));
                    
                    try {
                        const imageResults = await Promise.allSettled(
                            promptsToGenerate.map(p => generateAndUploadImage(p.prompt, p.key, session.user.id, p.aspectRatio))
                        );
                        ...
                        imageResults.forEach((result, index) => {
                            ...
                            if (result.status === 'fulfilled') {
                                newImages[key] = result.value; // This is now a URL, not base64
                                console.log(`✅ Image generated and uploaded successfully for ${key}`);
                            } else {
                        ...
                        console.log('📸 Image generation completed, setting final images:', Object.keys(finalImages));
                    } catch (imageError) {
                        console.error('❌ Critical error in image generation:', imageError);
                        // Don't let image generation failure kill the entire page generation
                        // Just use placeholder images and continue
                        const placeholderImages: ImageStore = {};
                        promptsToGenerate.forEach(p => {
                            placeholderImages[p.key] = 'https://picsum.photos/1280/720?grayscale';
                        });
                        finalImages = { ...foundImages, ...placeholderImages };
                        setImages(finalImages);
                        console.log('📸 Using placeholder images due to generation error');
                    }
                }
            } catch (imageProcessingError) {
                console.error('❌ Error in image processing, but page data is safe:', imageProcessingError);
                // Page generation was successful, just image processing failed
                // Set some basic placeholder images so the page doesn't break
                setImages({ 'placeholder': 'https://picsum.photos/1280/720?grayscale' });
            }
            ...
            let errorMessage = 'An unknown error occurred during page generation.';
            if (err?.message) {
                errorMessage = err.message;
            } else if (typeof err === 'string') {
                errorMessage = err;
            } else if (err && typeof err === 'object') {
                errorMessage = `Generation failed: ${JSON.stringify(err)}`;
            }
            
            setError(errorMessage);
            // Only clear the landingPageData if the main generation failed
            console.log('🚫 Clearing landingPageData due to main generation error');
            setLandingPageData(null);
        } finally {
    ...
    const handleSaveSection = async (sectionKey: string, newSectionData: any, newImages?: Record<string, string>) => {
        ...
        if (imagePromptsToUpdate.length > 0) {
            ...
            const results = await Promise.allSettled(
                imagePromptsToUpdate.map(p => generateAndUploadImage(p.prompt, p.key, session.user.id, p.aspectRatio))
            );
            
            results.forEach((res, i) => {
                const { key } = imagePromptsToUpdate[i];
                if (res.status === 'fulfilled') {
                    finalImages[key] = res.value; // This is now a URL, not base64
                } else {
    ...
    const handleSaveSeo = async (newSeoData: SEOData) => {
        ...
        if (newSeoData.ogImagePrompt && newSeoData.ogImagePrompt !== oldSeoData?.ogImagePrompt && !newSeoData.ogImagePrompt.startsWith('http')) {
            ...
            try {
                const imageUrl = await generateAndUploadImage(newSeoData.ogImagePrompt, 'og_image', session.user.id, '16:9');
                const newImages = { ...images, 'og_image': imageUrl};
                setImages(newImages);
            } catch (e) {
    ...
    const handleCreateNewPage = async (pageName: string) => {
        ...
        try {
            const newPage = await pageService.createPage(pageName, session.user.id);
            const { pages } = await refreshAllData();
            // Use the pages directly from refreshAllData return
    ...
    return (
        <div className={`w-screen h-screen flex flex-col font-sans ${themeMode} overflow-hidden`}>
            <FloatingControlPanel
                ...
                images={images}
                onImportFromUrl={(url) => console.log('Import from URL:', url)}
                onImportFromFile={(file) => console.log('Import from file:', file)}
                onImportTemplate={(id) => console.log('Import template:', id)}
                onDuplicatePage={() => console.log('Duplicate page')}
                onResetPage={handleStartOver}
                onUploadImage={(file) => console.log('Upload image:', file)}
                onManageMedia={() => console.log('Manage media')}
                onPreviewMode={() => console.log('Preview mode')}
                onFullscreenPreview={() => console.log('Fullscreen preview')}
                onShowAnalytics={() => console.log('Show analytics')}
                onShowIntegrations={() => console.log('Show integrations')}
            />
            ...
            <main className="flex-grow w-full h-0 overflow-hidden">
                <LandingPagePreview
                    ...
                    onRegenerateImage={async (key, p, ratio) => {
                         ...
                         try {
                            const imageUrl = await generateAndUploadImage(p, key, session.user.id, ratio);
